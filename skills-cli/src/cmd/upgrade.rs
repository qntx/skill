//! `skills self-update` command implementation.
//!
//! Downloads the latest release from GitHub and replaces the current binary.
//! This is a Rust-only capability — the TS CLI relies on npm for updates.
...
pub async fn run() -> Result<()> {
    ...
    let confirmed: bool = cliclack::confirm("Update now?")
        .initial_value(true)
        .interact()
        .into_diagnostic()?;
    ...
    let binary_data = download_asset(&asset_url).await?;
    ...
    let _ = cliclack::outro(format!(
        "{GREEN}Updated to {latest}!{RESET} {DIM}Restart your shell for changes to take effect.{RESET}"
    ));
...
fn replace_current_binary(new_binary: &[u8]) -> Result<()> {
    ...
    // Rename current → backup, write new, then clean up backup on next run.
    ...
    std::fs::write(&current_exe, new_binary).into_diagnostic()?;
