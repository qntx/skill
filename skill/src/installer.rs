...
pub async fn list_installed_skills(
    ...
    for scope in &scopes {
        ...
        for config in registry.all_configs() {
            let dir = agent_base_dir(config, registry, *scope, &cwd);
            if dir == canonical {
                continue;
            }
            scan_skills_dir_for_agent(&dir, *scope, &config.name, &mut skills_map).await;
        }
    }
...
async fn create_symlink(target: &Path, link_path: &Path) -> bool {
    ...
    #[cfg(unix)]
    {
        tokio::fs::symlink(target, link_path).await.is_ok()
    }

    #[cfg(windows)]
    {
        // Try junction first (no elevated permissions needed)
